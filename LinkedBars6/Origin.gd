...
func pinposA(p1, p2, L1, L2, p3):
	...
	# make a choice of which of the solutions to return
...
func pinposN(p1, p2, L1, L2, p3):
	...
	var iter = 0;
	while(v.length() > 1e-6 && iter < 5): # Check of norm of Residual is less than tolerance
		# Compute the adjoint of the jacobian
		J1 = Vector2((y-y2)*2, -(y-y1)*2); J2 = Vector2(-(x-x2)*2, (x-x1)*2);
		# Compute the determinant ofthe jacobian
		D = J1.x*J2.y - J1.y*J2.x; 
		# Perform Newton Step Update
		x -= J1.dot(v)/D;  y -= J2.dot(v)/D; 
		# Recompute the new residual
		v = Vector2((x-x1)**2 + (y-y1)**2 - L1**2, (x-x2)**2 + (y-y2)**2 - L2**2); 
		iter += 1;
	v3 = Vector2(x, y)
	return v3;
